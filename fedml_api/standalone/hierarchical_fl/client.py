...
class Client(Client):
    ...
    def train(self, global_round_idx, group_round_idx, w):
        self.model_trainer.load_state_dict(w)
        self.model_trainer.to(self.device)

        if self.args.client_optimizer == "sgd":
            optimizer = torch.optim.SGD(self.model_trainer.parameters(), lr=self.args.lr)
        else:
            optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, self.model_trainer.parameters()), lr=self.args.lr,
        ...
        for epoch in range(self.args.epochs):
            for x, labels in self.local_training_data:
                ...
                log_probs = self.model_trainer(x)
            ...
            if global_epoch % self.args.frequency_of_the_test == 0 or epoch == self.args.epochs-1:
                w_list.append((global_epoch, copy.deepcopy(self.model_trainer.state_dict())))
        return w_list
